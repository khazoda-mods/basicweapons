 ...
 * As both are being loaded from a custom directory Minecraft doesn't handle them automatically.
...
public class ResourceAndDatapackCustomLoader implements RepositorySource {
  ...
  public void loadPacks(Consumer<Pack> packConsumer) {
    ...
    File[] packs = packsFolder.listFiles(file -> file.isDirectory()
    ...
    if (packs != null) {
      Arrays.sort(packs, Comparator.comparing(File::getName, String.CASE_INSENSITIVE_ORDER).thenComparing(File::getName));
      for (File packFile : packs) {
        String packId = "basicweapons:" + packFile.getName();
